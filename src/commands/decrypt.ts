...
/**
 * "Envy: Decrypt" — headless invocation.
 *
 ...
 * and tree view on success. The passphrase is never written to the Output
 * Channel.
 */
export async function handler(refresh: () => Promise<void>): Promise<void> {
    ...
    const result = await execEnvy(['decrypt'], cwd, {
        env: { ...process.env, ENVY_PASSPHRASE: pw },
    });

    if (result.exitCode === 0) {
        void vscode.window.showInformationMessage('Vault restored.');
        await refresh();
        return;
    }

    void vscode.window.showErrorMessage(
        result.stderr.trim() || 'Decryption failed.',
    );
}
